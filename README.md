...
rules:
  - name: "부품목록표"                                  # 시트명 접두어
    expected_headers: ["품명", "규격", "수량", "비고"]    # 1행 헤더 (비중 50)
    title_keywords: ["부품 목록", "수리부속"]             # 캡션/직전 문단 키워드 (비중 30)
    expected_cols: 4                                    # 열 개수 일치 (비중 10)
    cell_pattern: "\\d{4}-\\d{2}-\\d{3}"                # 셀 내 정규식 (비중 10)
    threshold: 80                                       # 일치율(%) 기준. 이 값 이상이면 선정
```

- **일치율 방식**: 규칙에 입력한 조건만으로 만점을 계산하고, 표가 얼마나 맞는지를
  0~100% 일치율로 환산합니다. 입력한 조건을 전부 만족하면 어떤 조합이든 **100%** 입니다.
  (예: 헤더만 입력 후 4/4 일치 → 100%, 3/4 일치 → 75%)
- 헤더/키워드 비교 시 공백은 무시됩니다 ("품 명" = "품명").
- 한 표가 여러 규칙에 걸리면 일치율이 가장 높은 규칙 하나로 선정됩니다.
...
3. `run.bat 샘플.hwpx` 실행 후 `output\extract.log` 의 일치율을 보고 threshold(%) 조정
   (엉뚱한 표가 뽑히면 90~100, 원하는 표가 빠지면 60~70)
